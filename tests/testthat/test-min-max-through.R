...
test_that("min_through works: min values properly calculated [MRG-MMAX-001]", {
  expect_equal(min_through(vec1), vec1_mins)
  expect_equal(min_through(vec2), vec2_mins)
})

test_that("max_through works: max values properly calculated [MRG-MMAX-002]", {
  expect_equal(max_through(vec1), vec1_maxes)
  expect_equal(max_through(vec2), vec2_maxes)
})

test_that("min and max_through works: NA's properly maintained [MRG-MMAX-003]", {
...
test_that("min and max_through works: Works with tidy pipe [MRG-MMAX-003]", {
