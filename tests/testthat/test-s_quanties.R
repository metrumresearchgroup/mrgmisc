test_that("s_quantiles expected output: Generates correct quantiles [MRG-SQUN-001]", {
...
test_that("s_quantiles unexpected input: Appropriate error occurs [MRG-SQUN-002]", {
  expect_error(sd_oral_richpk %>% dplyr::group_by(Gender, Time) %>% s_quantiles(Conc, c(0.05, 0.5, 1.95)))
  expect_error(sd_oral_richpk %>% dplyr::group_by(Gender, Time) %>% s_quantiles(Conc, c(-0.05, 0.5, 1.95)))
})

test_that("s_quantiles expected output: Works with variety of quantile inputs [MRG-SQUN-001]", {
