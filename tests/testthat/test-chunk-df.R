test_that("Error message occurs if input is not correct type [MRG-CKDF-002]", {
...
test_that("Expected number of chunks is created [MRG-CKDF-001]", {
...
test_that("Column added to dataframe if list set to false [MRG-CKDF-002]", {
  chk_df <- Theoph %>% dplyr::group_by(Subject) %>% chunk_df(.nchunks = 3, .as_list = FALSE)
  expect_equal(length(unique(chk_df$chunk__)), 3)
})

test_that("Chunk df with column name [MRG-CKDF-001]", {
