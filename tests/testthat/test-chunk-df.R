test_that("chunk_df unexpected input: Error message occurs if input is not correct type", {
...
test_that("chunk_df expected output: Expected number of chunks is created", {
...
test_that("chunk_df unexpected input: Column added to dataframe if list set to false", {
  chk_df <- Theoph %>% dplyr::group_by(Subject) %>% chunk_df(.nchunks = 3, .as_list = FALSE)
  expect_equal(length(unique(chk_df$chunk__)), 3)
})

test_that("chunk_df expected output: Chunk df with column name", {
