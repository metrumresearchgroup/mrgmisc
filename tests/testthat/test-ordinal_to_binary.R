...
test_that("ordinal_to_binary_ expected output: Expected number of binary flag variables are generated", {
...
test_that("ordinal_to_binary_ unique case: Only 1 binary flag made per row", {
  occvars2 <- df2 %>% ordinal_to_binary_("OCC")
  occvars2 <- occvars2 %>% dplyr::mutate(SUM = rowSums(dplyr::across(OCC1:OCC8)))
  expect_equal(min(occvars2$SUM), 1)
})

test_that("ordinal_to_binary_ expected output: Binary flags made in correct columns", {
  occvars2 <- df2 %>% ordinal_to_binary_("OCC")
...
test_that("ordinal_to_binary_ unique case: Function will not overwrite existing columns by default", {
...
test_that("ordinal_to_binary_ unique case: Function will overwrite when called explicitly", {
  occvars3 <- suppressWarnings(df3 %>% ordinal_to_binary_("OCC", overwrite = TRUE))
  expect_true(max(occvars3$OCC1) == 1)
})

test_that("ordinal_to_binary_ unique case: Prefix used for columns when specified", {
