...
test_that("Function will not overwrite existing columns by default", {
...
test_that("Function will overwrite when called explicitly", {
  occvars3 <- suppressWarnings(df3 %>% ordinal_to_binary_("OCC", overwrite = TRUE))
  expect_true(max(occvars3$OCC1) == 1)
})

test_that("prefix used for columns when specified", {
