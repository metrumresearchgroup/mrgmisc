...
test_that("ordinal_to_binary_ works: Expected number of binary flag variables are generated [MRG-ORBI-001]", {
...
test_that("ordinal_to_binary_ special case: Only 1 binary flag made per row [MRG-ORBI-002]", {
  occvars2 <- df2 %>% ordinal_to_binary_("OCC")
  occvars2 <- occvars2 %>% dplyr::mutate(SUM = rowSums(dplyr::across(OCC1:OCC8)))
  expect_equal(min(occvars2$SUM), 1)
})

test_that("ordinal_to_binary_ works: Binary flags made in correct columns [MRG-ORBI-001]", {
  occvars2 <- df2 %>% ordinal_to_binary_("OCC")
...
test_that("ordinal_to_binary_ special case: Function will not overwrite existing columns by default [MRG-ORBI-002]", {
...
test_that("ordinal_to_binary_ special case: Function will overwrite when called explicitly [MRG-ORBI-002]", {
  occvars3 <- suppressWarnings(df3 %>% ordinal_to_binary_("OCC", overwrite = TRUE))
  expect_true(max(occvars3$OCC1) == 1)
})

test_that("ordinal_to_binary_ special case: Prefix used for columns when specified [MRG-ORBI-002]", {
