...
test_that("set_bins works: Works with breaks out of order [MRG-STBN-001]", {
  x <- Theoph$conc
...
test_that("set_bins works: Works with 1 break provided [MRG-STBN-001]", {
  x <- Theoph$conc
...
test_that("set_bins works: Works with default 4 bins quartiles if break not specified [MRG-STBN-001]", {
  x <- Theoph$conc
  res <- set_bins(x) 
...
test_that("set_bins bounds: Lower_bound test: -Inf [MRG-STBN-002]", {
  x <- Theoph$conc
...
test_that("set_bins bounds: Lower_bound test: discrete min number, break[1] > lower_bound [MRG-STBN-002]", {
  x <- Theoph$conc
...
test_that("set_bins bounds: Lower_bound test: discrete number > actual min [MRG-STBN-002]", {
  x <- Theoph$conc
...
test_that("set_bins bounds: Upper_bound test: -Inf [MRG-STBN-002]", {
  x <- Theoph$conc
...
test_that("set_bins bounds: Discrete max number, break[n] > upper_bound [MRG-STBN-002]", {
  x <- Theoph$conc
...
test_that("set_bins bounds: Discrete number < actual max [MRG-STBN-002]", {
  x <- Theoph$conc
  xbreak <- stats::quantile(x, na.rm = T, probs= c(0.4, 0.5, 0.8))
...
test_that("set_bins inclusion: Inclusive = TRUE and upper bound = Inf, n bins = n breaks -1 [MRG-STBN-003]", {
  x <- Theoph$conc
...
test_that("set_bins inclusion: Inclusive = FALSE and upper bound = Inf, n bins = n breaks [MRG-STBN-003]", {
  x <- Theoph$conc
  xbreak <- stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
  xupper = Inf
...
test_that("set_bins inclusion: Inclusive = TRUE and discrete upper bound, n bins = n breaks -1 [MRG-STBN-003]", {
  x <- Theoph$conc
  xbreak <- stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
  xupper = max(x)-2
  res <- set_bins(x, breaks = xbreak, upper_bound = xupper, inclusive = TRUE)
...
test_that("set_bins inclusion: Inclusive = FALSE and discrete upper bound, n bins = n breaks -1 [MRG-STBN-003]", {
  x <- Theoph$conc
  xbreak <- stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
  xupper = max(x)-2
  res <- set_bins(x, breaks = xbreak, upper_bound = xupper, inclusive = FALSE)
...
# if val between c(min_between, max_between), then bin 1. Less than min_between, then bin 0. Greater than max_between, then bin 2
  test_that("set_bins between: Discrete number with between argument [MRG-STBN-004]", {
    x <- Theoph$conc
    min_between <- min(x) + 1
    max_between <- max(x) - 5
...
test_that("set_bins between: Discrete number with between argument all > max val [MRG-STBN-004]", {
  x <- Theoph$conc
  min_between <- max(x) + 1
  max_between <- max(x) + 5
  res <- set_bins(x,  between = c(min_between, max_between)) 
...
test_that("set_bins between: Discrete number with between argument all < min val [MRG-STBN-004]", {
  x <- Theoph$conc
  min_between <- min(x) + 1
  max_between <- min(x) - 5
  res <- set_bins(x,  between = c(min_between, max_between)) 
...
# error message if between length(between) != 2
  test_that("set_bins between: Error length(between) !=2 [MRG-STBN-004]", {
    x <- Theoph$conc
...
test_that("set_bins quiet: Quiet= false print message [MRG-STBN-005]", {
  x <- Theoph$conc
  ...
  test_that("set_bins quiet: Quiet= between argument and false print message[MRG-STBN-005]", {
    x <- Theoph$conc
    xbreaks = stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
