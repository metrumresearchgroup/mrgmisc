...
test_that("set_bins_df works: Works with breaks out of order [MRG-SBDF-001]", {
  x <- Theoph$conc
...
test_that("set_bins_df works: Works with 1 break provided [MRG-SBDF-001]", {
  x <- Theoph$conc
...
test_that("set_bins_df bounds: Lower_bound test: -Inf [MRG-SBDF-002]", {
  x <- Theoph$conc
...
test_that("set_bins_df bounds: Discrete min number, break[1] > lower_bound [MRG-SBDF-002]", {
  x <- Theoph$conc
...
test_that("set_bins_df bounds: Discrete number > actual min [MRG-SBDF-002]", {
  x <- Theoph$conc
...
test_that("set_bins_df bounds: -Inf [MRG-SBDF-002]", {
  x <- Theoph$conc
...
test_that("set_bins_df bounds: Discrete max number, break[n] > upper_bound [MRG-SBDF-002]", {
  x <- Theoph$conc
...
test_that("set_bins_df bounds: Discrete number < actual max [MRG-SBDF-002]", {
  x <- Theoph$conc
  xbreak <- stats::quantile(x, na.rm = T, probs= c(0.4, 0.5, 0.8))
...
test_that("set_bins_df inclusion: Inclusive = TRUE and upper bound = Inf, n bins = n breaks -1 [MRG-SBDF-003]", {
  x <- Theoph$conc
...
test_that("set_bins_df inclusion: Inclusive = FALSE and upper bound = Inf, n bins = n breaks [MRG-SBDF-003]", {
  x <- Theoph$conc
  xbreak <- stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
  xupper = Inf
...
test_that("set_bins_df inclusion: Inclusive = TRUE and discrete upper bound, n bins = n breaks -1 [MRG-SBDF-003]", {
  x <- Theoph$conc
  xbreak <- stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
  xupper = max(x)-2
  res <- set_bins_df(.df = Theoph, .x= "conc", breaks = xbreak, upper_bound = xupper, inclusive = TRUE)
...
test_that("set_bins_df inclusion: Inclusive = FALSE and discrete upper bound, n bins = n breaks -1 [MRG-SBDF-003]", {
  x <- Theoph$conc
  xbreak <- stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
  xupper = max(x)-2
  res <- set_bins_df(.df = Theoph, .x= "conc", breaks = xbreak, upper_bound = xupper, inclusive = FALSE)
...
# if val between c(min_between, max_between), then bin 1. Less than min_between, then bin 0. Greater than max_between, then bin 2
  test_that("set_bins_df between: Discrete number with between argument [MRG-SBDF-004]", {
    x <- Theoph$conc
    min_between <- min(x) + 1
    max_between <- max(x) - 5
...
test_that("set_bins_df inclusion: Discrete number with between argument all > max val [MRG-SBDF-004]", {
  x <- Theoph$conc
  min_between <- max(x) + 1
  max_between <- max(x) + 5
  res <- set_bins_df(.df = Theoph, .x= "conc",  between = c(min_between, max_between)) 
...
test_that("set_bins_df inclusion: Discrete number with between argument all < min val [MRG-SBDF-004]", {
  x <- Theoph$conc
  min_between <- min(x) + 1
  max_between <- min(x) - 5
  res <- set_bins_df(.df = Theoph, .x= "conc",  between = c(min_between, max_between)) 
...
# error message if between length(between) != 2
  test_that("set_bins_df inclusion: Error length(between) !=2 [MRG-SBDF-004]", {
...
test_that("set_bins_df quiet: Quiet= false print message [MRG-SBDF-005]", {
  x <- Theoph$conc
  xbreak = stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
  ...
  test_that("set_bins_df quiet: Quiet= between argument and false print message[MRG-SBDF-005]", {
    x <- Theoph$conc
    xbreak = stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
...
# Name and label arguments ------------------------------------------------
  ...
  test_that("set_bins_df name and label: Default [MRG-SBDF-006]", {
    x <- Theoph$conc
    .x= "conc"
  ...
  test_that("set_bins_df name and label: Specified name [MRG-SBDF-006]", {
    x <- Theoph$conc
    .name = "conc_category"
  ...
  test_that("set_bins_df name and label: Specified name and label [MRG-SBDF-006]", {
    x <- Theoph$conc
    .name = "conc_category"
    .label = "conc_ranges"
  ...
  test_that("set_bins_df name and label: Specified label [MRG-SBDF-006]", {
    x <- Theoph$conc
    .x= "conc"
    .label = "conc_ranges"
...
# Label column output -----------------------------------------------------
  ...
  test_that("set_bins_df name and label: Default works [MRG-SBDF-006]", {
    x <- Theoph$conc
    .x= "conc"
    res <- set_bins_df(.df = Theoph, .x= "conc", breaks = stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1)))
    expect_equal(
  ...
  test_that("set_bins_df name and label: Contents of label column with default lower and upper bounds [MRG-SBDF-006]", {
    x <- Theoph$conc
    xbreak = stats::quantile(x, na.rm = T, probs= c(0, 0.5, 1))
    res <- set_bins_df(.df = Theoph, .x= "conc", breaks= xbreak)
    xbin <- length(xbreak) + 1
  ...
  test_that("set_bins_df name and label: Contents of label column with specified lower and upper bounds [MRG-SBDF-006]", {
    x <- Theoph$conc
    xbreak <- stats::quantile(x, na.rm = T, probs= c(0.4, 0.5, 0.8))
    lbound = 2
    ubound = 9
