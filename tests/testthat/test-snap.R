test_that("snap expected output: Different type input vectors", {
...
test_that("snap expected output: Non-unqiue elements in c", {
  expect_equal(snap(c(1, 1, 1, 2, 3), 0.3), c(0.9, 0.9, 0.9, 2.1, 3.0))
})

test_that("snap expected output: Works with NA present", {
  expect_equal(snap(c(3,NA,5), c(2,3,6)), c(3, NA, 6))
})

test_that("snap expected output: Returns all NA if rule is 0", {
