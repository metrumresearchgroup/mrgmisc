test_that("snap works: Works with different type input vectors [MRG-SNAP-001]", {
...
test_that("snap works: Works with non-unqiue elements in c [MRG-SNAP-001]", {
  expect_equal(snap(c(1, 1, 1, 2, 3), 0.3), c(0.9, 0.9, 0.9, 2.1, 3.0))
})

test_that("snap works: Works with NA [MRG-SNAP-001]", {
  expect_equal(snap(c(3,NA,5), c(2,3,6)), c(3, NA, 6))
})

test_that("snap works: Returns all NA if rule is 0 [MRG-SNAP-001]", {
