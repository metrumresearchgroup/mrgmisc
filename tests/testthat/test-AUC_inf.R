...
test_that("AUC_inf works as intended [MRG-AUCI-001]", {
...
test_that("AUC_inf NA testing: Remove NA works [MRG-AUCI-002]", {
  df1 <- Theoph
  df1$conc[4] <- NA
...
test_that("AUC_inf NA testing: Correct output if all input NA [MRG-AUCI-002]", {
  df2 <- Theoph
  df2$conc <- NA_real_
  expect_equal(auc_inf(df2$Time, df2$conc), setNames(NA, "AUC0_inf"))
})

test_that("AUC_inf unexpected input testing: Correct output if all DV is 0 [MRG-AUCI-003]", {
  df2 <- Theoph
  df2$conc <- 0
  expect_equal(auc_inf(df2$Time, df2$conc), setNames(0, "AUC0_inf"))
})

test_that("AUC_inf unexpected input testing: Correct output if all time is 0 [MRG-AUCI-003]", {
  df2 <- Theoph
  df2$Time <- 0
