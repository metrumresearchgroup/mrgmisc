...
stratify_df <- function(df,
  ...
  if (nleft != 0) {
    ifelse(nleft > 0, {
      extras <- dplyr::sample_n(df, size = nleft, replace = TRUE)
...
#' Resampling with controlled stratifications
#' @param df data frame
...
resample_df <- function(df,
  ...
  if (is.numeric(strat_cols)) {
    message(
      "It looks you are trying to give a numeric value for strat_cols,
 ...
 For example resample_df(Theoph, 'Subject', n = 20 )"
  ...
  if (is.null(strat_cols)) {
    ...
    if (nrow(strat_key) != nrow(key)) {
      warning(
        "Non-unique keys introduced from stratification,
check that all keys only have one stratification variable associated
                                             "
      )
    }
    sample <-
      stratify_df(strat_key, strat_cols, n, replace = replace)
